...
-spec connect(inet:socket_address() | inet:hostname(), inet:port_number()) -> {ok , eredis_sync:conn()} | {error, error()}.

connect(Host, Port) ->
  connect(Host, Port, ?DEFAULT_TIMEOUT).

-spec connect(inet:socket_address() | inet:hostname(), inet:port_number(), timeout()) -> {ok , eredis_sync:conn()} | {error, error()}.
...
-spec connect_db(inet:socket_address() | inet:hostname(), inet:port_number(), non_neg_integer()) -> {ok, eredis_sync:conn()} | {error, error()}.

connect_db(Host, Port, Db) ->
  connect_db(Host, Port, Db, ?DEFAULT_TIMEOUT).

-spec connect_db(inet:socket_address() | inet:hostname(), inet:port_number(), non_neg_integer(), timeout()) -> {ok, eredis_sync:conn()} | {error, error()}.
